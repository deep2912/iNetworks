...
switch($_GET["action"]) {
	case "add":
		if(!empty($_POST["quantity"])) {
			$productByCode = $db_handle->runQuery("SELECT * FROM routers WHERE code='" . $_GET["code"] . "'");
...
<?php		
    foreach ($_SESSION["cart_item"] as $item){
		?>
				...
				<td style="text-align:center;border-bottom:#F0F0F0 1px solid;"><a href="router.php?action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction">Remove Item</a></td>
				</tr>
				<?php
